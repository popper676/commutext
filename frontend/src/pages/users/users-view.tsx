    ...
    <>
      ...
      <SectionMain>
        ...
        <CardBox>
          ...
          <>
            <p className={'block font-bold mb-2'}>Texts User</p>
            <CardBox
              className='mb-6 border border-gray-300 rounded overflow-hidden'
              hasTable
            >
              <div className='overflow-x-auto'>
                <table>
                  ...
                  <tbody>
                    {users.texts_user &&
                      Array.isArray(users.texts_user) &&
                      users.texts_user.map((item: any) => (
                        <tr
                          key={item.id}
                          onClick={() =>
                            router.push(`/texts/texts-view/?id=${item.id}`)
                          }
                        >
          ...
          <>
            <p className={'block font-bold mb-2'}>Translations User</p>
            <CardBox
              className='mb-6 border border-gray-300 rounded overflow-hidden'
              hasTable
            >
              <div className='overflow-x-auto'>
                <table>
                  ...
                  <tbody>
                    {users.translations_user &&
                      Array.isArray(users.translations_user) &&
                      users.translations_user.map((item: any) => (
                        <tr
                          key={item.id}
                          onClick={() =>
                            router.push(
                              `/translations/translations-view/?id=${item.id}`,
                            )
                          }
                        >
                          <td data-label='original_text'>
                            {item.original_text}
                          </td>

                          <td data-label='translated_text'>
              ...
              {!users?.translations_user?.length && (
                <div className={'text-center py-4'}>No data</div>
              )}
            </CardBox>
          </>
