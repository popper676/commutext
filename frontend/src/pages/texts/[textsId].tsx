...
import { update, fetch } from '../../stores/texts/textsSlice';
...
const EditTexts = () => {
  ...
  const { textsId } = router.query;

  useEffect(() => {
    dispatch(fetch({ id: textsId }));
  }, [textsId]);

  useEffect(() => {
    if (typeof texts === 'object') {
      setInitialValues(texts);
    }
  }, [texts]);
  ...
  const handleSubmit = async (data) => {
    await dispatch(update({ id: textsId, data }));
    await router.push('/texts/texts-list');
  };

  return (
    <>
      <Head>
        <title>{getPageTitle('Edit texts')}</title>
      </Head>
      <SectionMain>
...
EditTexts.getLayout = function getLayout(page: ReactElement) {
  return (
    <LayoutAuthenticated permission={'UPDATE_TEXTS'}>
      {page}
    </LayoutAuthenticated>
  );
};
